...
/**
 * Put your own github token here.
 */
...
async function waitForRateLimitReset() {
  ...
  if (resetTime > now) {
    const waitTime = resetTime - now + 15000; // Add 1 seconds buffer
...
/**
 * This function generates a list with all months between 2001 and 2023.
      ...
      for (let page = 1; page <= 10; page++) {
        ...
        if (up === 900) {
          response = await octokit.request("GET /search/repositories", {
            q: `language:${language} stars:>=900`,
            per_page: reposPerPage,
            page: page,
          });
        } else {
          response = await octokit.request("GET /search/repositories", {
            q: `language:${language} stars:${down}..${up}`,
            per_page: reposPerPage,
            page: page,
          });
        }
      ...
      if (intervalRepoC > 0 && intervalRepoC < 1000) {
        ...
        for (let date = 0; date <= dateArray.length - 2; date += 2) {
          for (let page = 1; page <= 10; page++) {
            ...
            if (up === 900) {
              response = await octokit.request("GET /search/repositories", {
                q: `language:${language} stars:>=900 created:${
                  dateArray[date]
                }..${dateArray[date + 1]}`,
                per_page: reposPerPage,
                page: page,
              ...
              response = await octokit.request("GET /search/repositories", {
                q: `language:${language} stars:${down}..${up} created:${
                  dateArray[date]
                }..${dateArray[date + 1]}`,
                per_page: reposPerPage,
                page: page,
...
async function saveRepos(languages, repoC) {
  ...
  try {
    ...
    for (lang of languages) {
      let counter = 0;
      filePathGlb = `./no_license/${lang}AnyLicense.json`;
      ...
      console.log(counter);
      await fs.appendFile(filePathGlb, jsonRepos + "]", "utf8");
      await wait(180000); // wait for next lang, sometimes github still throws secondary rate limit
    }
  } catch (error) {
